...
- Full CLI with argparse: `-o/--output`, `-r/--run`, `-v/--verbose`, `-d/--debug`, `-V/--version`
- `p2w -r` compiles and runs a Python file in one command (via wasm-tools + Node.js)

### Fixed

- README now references `wasm-tools` instead of `wabt` (which doesn't support WASM GC)

### Changed

- Refactored type inference to use class-level constants for operator/type mappings
- Improved README with step-by-step usage example and CLI reference

## [0.2.1] - 2026-03-02

### Changed

- Refactored codegen modules to reduce code duplication
