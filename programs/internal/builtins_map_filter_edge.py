"""Test map and filter edge cases."""
...
print("map with conditional lambda:")
result = list(map(lambda x: x if x > 0 else -x, [-3, -1, 0, 2, 4]))
print(result)  # [3, 1, 0, 2, 4]
...
print("map with two iterables:")
result = list(map(lambda x, y: x + y, [1, 2, 3], [10, 20, 30]))
print(result)  # [11, 22, 33]

print("map with two iterables different lengths:")
result = list(map(lambda x, y: x * y, [1, 2, 3, 4, 5], [10, 20]))
print(result)  # [10, 40] (stops at shortest)

print("map with three iterables:")
result = list(map(lambda x, y, z: x + y + z, [1, 2], [10, 20], [100, 200]))
print(result)  # [111, 222]
...
print(result)  # [1, 'a', True, [1]]

print("filter None on numbers:")
...
print("filter with type check:")
mixed = [1, "a", 2, "b", 3, "c"]
result = list(filter(lambda x: isinstance(x, int), mixed))
print(result)  # [1, 2, 3]

result = list(filter(lambda x: isinstance(x, str), mixed))
print(result)  # ['a', 'b', 'c']

print("filter on string:")
result = list(filter(lambda c: c != "l", "hello"))
print(result)  # ['h', 'e', 'o']
...
print("filter with complex condition:")
result = list(filter(lambda x: x > 0 and x % 2 == 0, [-4, -2, 0, 2, 4, 6]))
print(result)  # [2, 4, 6]
...
# Keep evens, double them, keep those divisible by 3
result = list(filter(
    lambda x: x % 3 == 0,
    map(
        lambda x: x * 2,
...
print(result)  # [12] (6*2=12 is divisible by 3)
...
print("practical: parse and filter:")
data = ["10", "20", "invalid", "30", "40"]
# Filter valid numbers, convert to int
def try_int(s):
    try:
        return int(s)
    except ValueError:
...
parsed = list(filter(None, map(try_int, data)))
print(parsed)  # [10, 20, 30, 40]

print("practical: transform strings:")
words = ["  hello  ", "WORLD", "  Python  "]
result = list(map(lambda s: s.strip().lower(), words))
...
print("builtins_map_filter_edge tests done")
